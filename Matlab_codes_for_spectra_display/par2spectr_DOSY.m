function Spec_grid = par2spectr_DOSY(DiffCoef,Spectrum, idx_freq, sgm_d, sgm_f, diff_v, ppm);
...
if nargin < 7||isempty(ppm)
    Nf = 1e3;
    ppm = 1: Nf;
else
    Nf = length(ppm);
end
if nargin < 6||isempty(diff_v)
    Nd = 1e2;
    diff_v = linspace(min(DiffCoef),max(DiffCoef),Nd);
else
    Nd = length(diff_v);
end
if nargin < 5||isempty(sgm_f)
    sgm_f = 1;
end
if nargin < 4||isempty(sgm_d)
